...
class SearchAvailabilityPresenter extends ActionPresenter
{
    ...
    private function GetSearchRange()
    {
        ...
        if ($range == 'tomorrow') {
            return new DateRange($today->AddDays(1)->ToTimezone($timezone)->GetDate(), $today->AddDays(2)->ToTimezone($timezone)->GetDate());
        }
        ...
        if ($range == 'thisweek') {
            ...
            $startDate = $today->AddDays($adjustedDays)->ToTimezone($timezone)->GetDate();
